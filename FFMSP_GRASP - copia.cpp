...
class GRASP{
	private:
		...
		double th, det;
		int n, m, ti, tMAx, intentosExtra, tuning;
		minstd_rand rng;
		...
		char encontrarMejorBase(int col){
			...
			if(rng()%100 < probMAx*100) return maximos[ rng() % maximos.size() ];
		...
		pair<string, int> busquedaLocal(pair<string, int> solucion){
			...
			while(intentos && time(NULL) - ti <= tMAx && mejorCal != n){
				...
				for(int col: indices){  // para cada columna
					...
					for(char base: bases){	 // para cada base a testear
						calidadBase[base] = 0;
						if(base == sol[col] && rng()%(int)(a*m) == 0) continue;
	...
	public:
		GRASP(string instancia, double threshold, double determinismo, int tiempoMaximo, int intentos, int tun){	
		...
		pair<int, int> iniciar(){
			...
			while(time(NULL) - ti <= tMAx && solActual.second != n){
				...
				if(solNueva.second > solActual.second){
					solActual = solNueva;
					tf = time(NULL) - ti;
					if(tuning == 0) cout << "Nueva solucion: " << solNueva.second << "  Tiempo: " << tf << endl;
				}
			}

			if(tuning == 0) cout << "Mejor solucion: " << solActual.second << "  Tiempo usado: " << tf << endl;
			else cout << -1*solActual.second << endl;
...
int main(int argc, char *argv[]){
	...
	double threshold = 0.80, determinismo = 0.23;
	int tiempoMaximo = 90, intentosExtra = 30, tuning = 0;
	a = 4.12;
	probMAx = 0.1;
